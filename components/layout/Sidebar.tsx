  ...
  const content = (
    <View className="flex h-full w-72 border-r border-gray-200 bg-white px-4 pb-6 pt-8 shadow-2xl dark:border-gray-700 dark:bg-gray-950">
      <ScrollView contentContainerStyle={{ paddingBottom: 24 }} showsVerticalScrollIndicator={false}>
        ...
        {/* Invoices Link - Only visible when authenticated */}
        {isAuthenticated && (
          <Pressable
            accessibilityRole="button"
            onPress={() => handleNavigate('/(authenticated)/invoices')}
            className={`mb-2 flex-row items-center gap-3 rounded-xl px-3 py-3 text-sm ${
            ...
            <MaterialIcons
              name="receipt"
              size={20}
              color={pathname?.includes('/invoices') ? '#7b1c1c' : colorScheme === 'dark' ? '#e5e7eb' : '#374151'}
            />
            <Text
              className={`font-inter text-sm ${
                pathname?.includes('/invoices')
